...
static string buildOutcomeLine(const ActionResult& r) {
    ...
    if (r.foodDelta != 0)
        s += string(" | Food ")
           + (r.foodDelta > 0 ? "+" : "") + to_string(r.foodDelta);
    if (r.waterDelta != 0)
        s += string(" | Water ")
           + (r.waterDelta > 0 ? "+" : "") + to_string(r.waterDelta);
    if (r.staminaDelta != 0)
        s += string(" | Stamina ")
           + (r.staminaDelta > 0 ? "+" : "") + to_string(r.staminaDelta);
    if (r.sanityDelta != 0)
        s += string(" | Sanity ")
           + (r.sanityDelta > 0 ? "+" : "") + to_string(r.sanityDelta);
    if (r.durabilityDelta != 0)
        s += string(" | Dur ")
           + (r.durabilityDelta > 0 ? "+" : "") + to_string(r.durabilityDelta);
    return s;
}
...
int main() {
    ...
    while (true) {
        ...
        if (loadChoice == 1 && SaveSystem::hasSaveFile()) {
            if (SaveSystem::peekSave(diff, hasPet)) loaded = true;
        }
        ...
        if (loaded) SaveSystem::loadGame(game, g_sf);
